...
/// @param error 错误信息
/// @return 返回数据数组
+ (NSArray<DBPlayerData*>*)kj_insertData:(NSString*)dbid insert:(void(^)(DBPlayerData *data))insert error:(NSError**)error{
...
/// @param update 更新回调
/// @return 返回数据数组
+ (NSArray<DBPlayerData*>*)kj_updateData:(NSString*)dbid update:(void(^)(DBPlayerData *data, BOOL * stop))update{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:kDBPlayerData];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"dbid = %@", dbid];
...
+ (NSArray<DBPlayerData*>*)kj_checkData:(NSString*)dbid{
    ...
    return [[DBPlayerDataManager context] executeFetchRequest:request error:nil];
}
/// 指定条件查询数据
/// @param data 数据
...
/// @return 返回数据数组
+ (NSArray<DBPlayerData*>*)kj_checkAppointData:(DBPlayerData *)data fromat:(NSString *)fromat threshold:(id)threshold{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:kDBPlayerData];
