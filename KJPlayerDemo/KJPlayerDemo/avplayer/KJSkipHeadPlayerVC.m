...
- (void)viewDidLoad {
    ...
    self.player.kVideoSkipTime(^(KJPlayerVideoSkipState skipState) {
        if (skipState == KJPlayerVideoSkipStateHead) {
            [weakself.basePlayerView.hintTextLayer kj_displayHintText:@"跳过片头，自动播放" time:5 position:KJPlayerHintPositionLeftBottom];
        }
    }, 18, 0);
...
- (void)kj_player:(KJBasePlayer*)player state:(KJPlayerState)state{
    if (state == KJPlayerStateBuffering) {
        ...
        [self.basePlayerView.loadingLayer kj_startAnimation];
        [self.basePlayerView.hintTextLayer kj_displayHintText:@"暂停ing" time:5 position:KJPlayerHintPositionLeftBottom];
    }
}
/* 播放进度 */
